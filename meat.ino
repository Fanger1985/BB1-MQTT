...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
    ...
    if (topicStr == "esp32/commands/moveForward") {
        moveForward();
        Serial.println("Command received: Move Forward");
    } else if (topicStr == "esp32/commands/moveBackward") {
        moveBackward();
        Serial.println("Command received: Move Backward");
    } else if (topicStr == "esp32/commands/stop") {
        stopMotors();
        Serial.println("Command received: Stop Motors");
    } else if (topicStr == "esp32/commands/spinLeft") {
        spinLeft();
        Serial.println("Command received: Spin Left");
    } else if (topicStr == "esp32/commands/spinRight") {
        spinRight();
        Serial.println("Command received: Spin Right");
    } else if (topicStr == "esp32/commands/autoMode") {
        isManualControl = false;
        Serial.println("Command received: Switch to Autonomous Mode");
    } else if (topicStr == "esp32/commands/manualMode") {
        isManualControl = true;
        Serial.println("Command received: Switch to Manual Mode");
    } else if (topicStr == "esp32/commands/dance") {
        danceRoutine();
        Serial.println("Command received: Perform Dance Routine");
    } else if (topicStr.startsWith("esp32/commands/expressEmotion")) {
        // Expecting the format: esp32/commands/expressEmotion/happy
        String emotion = topicStr.substring(topicStr.lastIndexOf('/') + 1);
        expressEmotion(emotion);
        Serial.print("Command received: Express Emotion - ");
...
// Additional function to publish sensor data
void publishSensorData() {
    ...
    String sensorData;
    serializeJson(doc, sensorData); // Serialize JSON data to string

    mqttClient.publish("esp32/status/sensors", sensorData.c_str()); // Publish sensor data
...
server.on("/sensors", HTTP_GET, []() {
    ...
    server.begin(); // Start HTTP server
        setupMQTT();
        reconnectMQTT(); // Attempt to connect to the MQTT broker
}
...
void updateSensorsAndReact() {
    ...
    // Example reaction based on distance
    if (distance < 30) {
        Serial.println("Close obstacle detected, stopping motors.");
        stopMotors();
    } else {
        Serial.println("No close obstacle, continuing operation.");
    }
}
...
void loop() {
    ...
    // Check for sudden movement, which could indicate a fall or rapid movement
    if (abs(ax) > 15000 || abs(ay) > 15000 || abs(az) > 15000) {
        Serial.println("Sudden movement detected! Stopping motors for safety.");
        stopMotors(); // Safety measure to avoid damage
    }

    // Handle automatic control based on sensor inputs
    if (!isManualControl) {
        int pirState = digitalRead(PIR_SENSOR_PIN); // Check if a human is detected
        int followDistance = getUltrasonicDistance(); // Distance to the nearest object

        // Human detection logic
        if (pirState == HIGH) { // If human is detected
            Serial.println("Human detected! Expressing happiness...");
            expressEmotion("happy"); // React with an emotion

            // Decision making based on the distance to the human
            if (followDistance > 100) {
                Serial.println("Safe distance detected. Moving forward.");
                moveForward();
                delay(500); // Short movement delay
            } else if (followDistance < 30) {
                Serial.println("Too close! Stopping motors to avoid collision.");
                stopMotors(); // Prevent collisions
            }
        } else { // No human detected, continue exploring
            Serial.println("No human detected. Exploring the environment autonomously.");
            autoMove(); // Continue exploring and mapping
        }

        // Obstacle avoidance logic
        if (followDistance < 30) {
            Serial.println("Obstacle detected close by, initiating evasive maneuvers.");
            reactToCloseObstacle(); // Evasive action
        } else if (followDistance >= 100) {
            Serial.println("Obstacle cleared, resuming normal movement.");
            moveForward(); // Resume normal movement
        }

        updateMap(); // Update the environment map with new sensor data
            mqttClient.loop(); // This should be called regularly to maintain the connection and handle incoming messages
                publishSensorData(); // Regularly publish sensor data
    }

    // Periodic sensor data reporting
    static unsigned long lastReportTime = 0;
    const unsigned long reportInterval = 1000; // Report every 1000 milliseconds
    unsigned long lastSensorUpdateTime = 0; // Initialize last sensor update time
const unsigned long SENSOR_UPDATE_INTERVAL = 1000; // Time interval to update sensors

    if (millis() - lastReportTime >= reportInterval) {
        lastReportTime = millis();
        reportSensorData(); // Function to handle sensor data reporting
    }
    // Perform sensor updates and movement checks periodically
    unsigned long currentMillis = millis();
    if (currentMillis - lastSensorUpdateTime > SENSOR_UPDATE_INTERVAL) {
        lastSensorUpdateTime = currentMillis;
        updateSensorsAndReact();  // A function to handle sensor readings and reactions
    }
}
...
void reportSensorData() {
    ...
    IR_Left_Value = digitalRead(IR_LEFT);
    IR_Right_Value = digitalRead(IR_RIGHT);
    distance = getUltrasonicDistance(); // Read ultrasonic distance sensor
    ...
    // Record sensor data and update the map with current readings
    recordPath(distance, "sensor update"); // Record sensor data
    updateMap(); // Update environment map based on new data
}
...
void autoMove() {
    ...
    // Check for obstacles
    if (distance < 30) { // Close to an obstacle
        ...
        adjustBehaviorBasedOnScore(); // Adjust behavior based on score and environment
        return;
    }

    // If no obstacle detected, continue with normal behavior
...
void exploreEnvironment() {
    ...
    if (frontDistance < 30) {  // Too close to something, change direction
        reactToCloseObstacle();  // Handle the obstacle appropriately
        recordPath(frontDistance, "reactToCloseObstacle");  // Record the action
        return;  // Exit function to avoid moving forward
    }
    ...
    if (random(0, 10) < 2) {  // 20% chance to spin left
        ...
        delay(1000);
        return;  // Exit function after spinning
